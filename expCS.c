...
void expCS(){
  ...
  double dW=M_PI*pow(1.55,2)/pow(60,2); //solid angle[sr]
  ...
  //double Ierror=0.5*pow(10,-9); //current error[A]
  double r=M_PI/180; //degree to radian
  double CMangle;
  static double counts[160],I[160],Ierror[160]; 
  ...
  CMcross[angle]=(1+cos(angle*r)/A)/(pow(((2*(cos(angle*r))/A)+1+(1/(A*A))) ,1.5))*cross[angle];
  ...
  if(0<=angle && angle<=90){
  CMangle=atan(sin(r*(double)angle)/(cos(r*(double)angle)-(1/A)))/r;
  }else if(angle>90){
  CMangle=(M_PI+atan(sin(r*(double)angle)/(cos(r*(double)angle)-(1/A))))/r;
  }
  ...
  for(int i=0;i<10000;i++){
	...
	if(YesorNo=="yes" || YesorNo=="Yes" || YesorNo=="YES"){
		ofs << CMangle << " " << CMcross[angle] << " "  << CSerror[angle] << "\n";
		break;
