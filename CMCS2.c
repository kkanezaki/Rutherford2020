...
void CMCS2(){
	...
	/*d_t=d_t*pow(10,-4); //μm to cm
	d_terror=d_terror*pow(10,-4);
	*/
	...
	if(angle==140){
		...
		cross[angle]=counts[angle]*e*A/dW140/I[angle]/time/d_t/Na;
		CSerror[angle]=pow( pow(cross[angle],2)/counts[angle] + pow(cross[angle],2)*pow(I[angle],-2)*pow(Ierror[angle],2) + pow(cross[angle],2)*d_terror*d_terror/(d_t*d_t) ,0.5);
		...
		cross[angle]=counts[angle]*e*A/dW/I[angle]/time/d_t/Na;
		CSerror[angle]=pow( pow(cross[angle],2)/counts[angle] + pow(cross[angle],2)*pow(I[angle],-2)*pow(Ierror[angle],2) + pow(cross[angle],2)*d_terror*d_terror/(d_t*d_t) ,0.5);
